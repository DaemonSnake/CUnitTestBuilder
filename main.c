...
** Last update Sun Nov 29 03:22:07 2015 penava_b
*/
...
static int     	call(int which)
{
  ...
  if (tmp->use_set != 0)
    return which;
  asprintf(&sourceName, "_test_%s.c", tmp->file_name);
  ...
  tmp->use_set = 42;
  asprintf(&tmpRes, "gcc %s %s -o _test_%s_%d", sourceName, tmp->flags, tmp->file_name, which);
  if (system(tmpRes) != 0)
...
\n\
for line in `ls | grep _test_ | sort -t '_' -k 4n`;\n\
do\n\
    number=`echo $line | awk -F '_' '{print $NF}'`\n\
    name=`echo $line | awk -F '_' '{print $3}'`\n\
    ./$line;\n\
    if [ $? == 0 ]\n\
    then\n\
